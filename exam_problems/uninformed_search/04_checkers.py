...
class Balls(Problem):
    def __init__(self, n, obstacles, initial, goal=None):
    ...
    def successor(self, state):
        ...
        for (x, y) in state:
            if (x - 1, y + 1) in state and x - 2 >= 0 and y + 2 >= 0 and x - 2 < self.n and y + 2 < self.n:
                new_state = list(state)
                new_state.remove((x - 1, y + 1))
                i = new_state.index((x, y))
                new_state[i] = (x - 2, y + 2)
            ...
            if (x - 1, y) in state and x - 2 >= 0 and y >= 0 and x - 2 < self.n and y < self.n:
                new_state = list(state)
...
if __name__ == "__main__":
    ...
    for i in range(l):
        entry = input().split(",")
        initial.append((int(entry[0]), int(entry[1])))

    obstacles = []
    l = int(input())
    for i in range(l):
        entry = input().split(",")
        obstacles.append((int(entry[0]), int(entry[1])))

    balls = Balls(n, tuple(obstacles), tuple(initial), (n // 2, n - 1))
    print(breadth_first_graph_search(balls).solution())

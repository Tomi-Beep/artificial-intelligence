...
class Table(Problem):
    def __init__(self, n, initial, goal=None):
        super().__init__(tuple(initial), goal)
        self.n = n
    ...
    def successor(self, state):
        ...
        for row in range(self.n):
            for col in range(self.n):
                ...
                # up
                if row > 0:
                    up_index = (row - 1) * self.n + col
                    new_state[up_index] = 1 - new_state[up_index]

                # down
                if row < self.n - 1:
                    down_index = (row + 1) * self.n + col
                    new_state[down_index] = 1 - new_state[down_index]

                # left
                if col > 0:
                    left_index = row * self.n + (col - 1)
                    new_state[left_index] = 1 - new_state[left_index]

                # right
                if col < self.n - 1:
                    right_index = row * self.n + (col + 1)
                    new_state[right_index] = 1 - new_state[right_index]

                action = f"x: {row}, y: {col}"
...
if __name__ == "__main__":
    ...
    problem = Table(n, initial, tuple(1 for _ in range(n * n)))

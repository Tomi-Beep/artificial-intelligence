...
class Blocks(Problem):
    ...
    def successor(self, state):
        successors = dict()
        for (by, to) in [(i, j) for i in range(len(state)) for j in range(len(state)) if i != j]:
            ...
            if by_top <= to_top or to_top == -1:
                # ((3,2,1),(),())
                new_state = list(list(pillar) for pillar in state)

                new_state[by].pop()
                new_state[to].append(by_top)

                new_state = tuple(tuple(pillar) for pillar in new_state)

                successors[f"MOVE TOP BLOCK FROM PILLAR {by + 1} TO PILLAR {to + 1}"] = new_state
...
if __name__ == "__main__":
    ...
    for i in range(len(initial)):
        initial[i] = tuple(map(int, initial[i].split(","))) if initial[i] else ()
        goal[i] = tuple(map(int, goal[i].split(","))) if goal[i] else ()

    blocks = Blocks(tuple(initial), tuple(goal))
    solution = breadth_first_graph_search(blocks)
